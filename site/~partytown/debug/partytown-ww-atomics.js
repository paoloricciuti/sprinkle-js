...
(self => {
    ...
    const getPartytownScript = () => `<script src="${partytownLibUrl("partytown.js?v=0.4.5")}"><\/script>`;
        ...
        const WorkerWindow = defineConstructorName(class extends WorkerBase {
            constructor() {
                    ...
                    if (!hasInitializedMedia) {
                        (() => {
                            if (!webWorkerCtx.$initWindowMedia$) {
                                ...
                                webWorkerCtx.$importScripts$(partytownLibUrl("partytown-media.js?v=0.4.5"));
                ...
                let $createNode$ = (nodeName, instanceId, namespace) => {
                    ...
                    const NodeCstr = nodeCstrs[nodeName] ? nodeCstrs[nodeName] : nodeName.includes("-") ? nodeCstrs.UNKNOWN : nodeCstrs.DIV;
                    return new NodeCstr($winId$, instanceId, EMPTY_ARRAY, nodeName, namespace);
                };
    ...
    const receiveMessageFromSandboxToWorker = ev => {
        ...
        if (webWorkerCtx.$isInitialized$) {
            if (5 === msgType) {
                ...
                12 === msgType ? environments[msgValue].$visibilityState$ = msg[2] : 11 === msgType && (environments[msgValue].$location$.href = msg[2]);
            }
        } else if (1 === msgType) {
            (initWebWorkerData => {
